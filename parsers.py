...
class PravdaSotrudnikovParser:
    ...
    def _parse_single_review(self, block):
        ...
        ratings2 = {}
        for item in block.find_all('div', class_='company-reviews-list-item-ratings2-item'):
            text = item.get_text(strip=True)
            if ':' in text:
                key, val = text.split(':', 1)
                ratings2[key.strip()] = val.strip()
        category_ratings = {}
        for item in block.find_all('div', class_='company-reviews-list-item-ratings-item'):
            label_elem = item.find('span', class_='company-reviews-list-item-ratings-item-label')
            if not label_elem:
                continue
            label = label_elem.get_text(strip=True).replace(':', '')
            star_span = item.find('span', class_='rating-autostars')
            if star_span:
                rating_val = star_span.get('data-rating', '')
                category_ratings[label] = rating_val
        return {
            'author': author,
            'status': status,
            'city': city,
            ...
            'pros': pros,
            'cons': cons,
            'salary_type': ratings2.get('Зарплата', ''),
            'salary_market': ratings2.get('Соответствие рынку', ''),
            'impression': ratings2.get('Общее впечатление', ''),
    ...
    def _extract_review_text(self, block):
        ...
        if not msg_div:
            return ''
        collapsible = msg_div.find('div', class_='collapsible-body')
        if collapsible:
